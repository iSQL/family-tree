...
### Proposals from relatives (`/predlog/:token` → `/settings/proposals`)

Git "pull request" model for adding people. The admin creates an invite link (token with a **mandatory** 7/30/90-day expiry, revocable) on `/settings/proposals`, which is reachable **only from a Settings card** (no nav tab) and shown for `useCanWrite()`. Anyone with the link can, without logging in, view the tree (thumbnails via the token-gated `/api/proposals/public/tokens/:token/photos/:uuid`) and submit a proposal; nothing touches `persons`/`unions` until the admin approves.

- Server: the public router is mounted **before** `requireAuth` and is rate-limited (reads 120/15 min, submits 10/h per IP); a proposal holds at most 50 persons/unions/parent links, and a token at most 20 pending proposals. The admin router is behind `requireFullAccess`. Tables `proposal_tokens` and `proposals` (migration 3); timestamps are written as ISO UTC by the service.
- Payload (`ProposalData`, JSON in `proposals.data`): new `persons` with a `temp_id`; references are `PersonRef = number | string` (existing id or temp_id). `unions` between any refs. `parent_links` make a new person the father/mother of an **existing** person — only into an empty slot (a proposal never edits existing people otherwise). `refs` is a server-made snapshot (name + birth date) of every referenced existing person.
- [shared/src/proposalCheck.ts](shared/src/proposalCheck.ts) (`checkProposal`) is the single pure validator — server on submit (422) and approve (409, details in `ApiErrorBody.issues`), client live. Errors: unknown refs, self/same parents, parent gender, taken parent slot, cycles, missing persons and **stale refs** (snapshot ≠ current person, e.g. after a backup restore or GEDCOM replace reused the id). Warnings: `possible_duplicate` (`findPossibleDuplicates` — diacritics-insensitive first name, surname/maiden name, birth year ±2) and already existing unions (skipped on approve).
- Approve ([services/proposalService.ts](server/src/services/proposalService.ts)) runs in one transaction through the normal services (`createPerson`, `updatePerson`, `createUnion` from [services/unionService.ts](server/src/services/unionService.ts)), so the usual parent/cycle checks apply; new persons are inserted parents-first. The admin resolves duplicates with `merges` (`temp_id` → existing `person_id`): that person isn't created and every reference to it points at the existing one.
- Client: contributor page [routes/ContributorProposal.tsx](client/src/routes/ContributorProposal.tsx) (outside `AuthGuard`; draft in localStorage via [lib/proposalDraft.ts](client/src/lib/proposalDraft.ts); new persons are shown in `TreeCanvas` with negative ids and the `photoUrl` prop points thumbnails at the public route), admin page [routes/Proposals.tsx](client/src/routes/Proposals.tsx) + `components/proposals/`, hooks in [hooks/useProposals.ts](client/src/hooks/useProposals.ts).
...
To back up, copy the whole `data/` directory. Three backup surfaces exist, in increasing completeness: **GEDCOM export** (`/api/gedcom/export`) — portable standard, genealogy data only (no photos, no `is_family_head`, lossy on partnerships/end-reasons); the automatic **daily DB copy** (`backups/`, DB only, no photos); and the **full backup** (`/api/backup`, [services/fullBackup.ts](server/src/services/fullBackup.ts)) — a ZIP with a JSON dump of the tables (persons, unions, proposal tokens, proposals — all columns) **plus every photo file**, restorable in place. Full backup is **admin-only** (`requireFullAccess`) and restore is destructive (wipes then two-pass re-inserts to satisfy parent FKs, then swaps the photos dir); a backup made before proposals existed leaves the current proposals untouched. ZIP via `fflate` (`zipSync`/`unzipSync`, store-only). UI lives on the GEDCOM page ([routes/Gedcom.tsx](client/src/routes/Gedcom.tsx)), shown only for `useCanWrite()`.
